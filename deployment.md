...
* `FIRST_USER`: The email address of the first user.
...
* `FIRST_USER_PASSWORD`: The password of the first user.
...
You can generate secure values for `SECRET_KEY` and `FIRST_USER_PASSWORD` with:
...
The workflow runs database migrations and creates the first user before deploying. In the repository's **Settings** > **Secrets and variables** > **Actions** page, add these repository variables:
...
2. Runs `backend/scripts/prestart.sh` to apply database migrations and create the first user.
